...
using System.Linq;
using System.Text;
using Uploadino.BootloaderProgrammers.Protocols.AVR109.Messages;
using Uploadino.Hardware;
using Uploadino.Hardware.Memory;
using RJCP.IO.Ports;

namespace Uploadino.BootloaderProgrammers.Protocols.AVR109
{
    internal class Avr109BootloaderProgrammer : ArduinoBootloaderProgrammer
    {
        ...
        public override void InitializeDevice()
        {
            ...
            if (checkBlockResponse == null)
                throw new UploadinoException("Unable to retrieve block support!");
            if (!checkBlockResponse.HasBlockSupport)
                throw new UploadinoException("Block support is not supported!");
            ...
            if (response != Constants.CarriageReturn)
                throw new UploadinoException("Unable to execute select device type command!");
        }
        ...
        public override void EnableProgrammingMode()
        {
            ...
            var response = ReceiveNext();
            if (response != Constants.CarriageReturn)
                throw new UploadinoException("Unable to enter programming mode!");
        }
        ...
        public override void LoadAddress(IMemory memory, int offset)
        {
            ...
            var response = ReceiveNext();
            if (response != Constants.CarriageReturn)
                throw new UploadinoException("Unable to execute set address request!");
        }
        ...
        public override void ExecuteWritePage(IMemory memory, int offset, byte[] bytes)
        {
            ...
            var response = ReceiveNext();
            if (response != Constants.CarriageReturn)
                throw new UploadinoException("Unable to execute write page!");
        }
        ...
        public override void LeaveProgrammingMode()
        {
            ...
            if (leaveProgModeResp != Constants.CarriageReturn)
                throw new UploadinoException("Unable to leave programming mode!");

            Send(new ExitBootLoaderRequest());
            var exitBootloaderResp = ReceiveNext();
            if (exitBootloaderResp != Constants.CarriageReturn)
                throw new UploadinoException("Unable to exit boot loader!");
        }
    }
}